...
def main():
    ...
    similarity_df = pd.read_csv('./curated_category_similarities.csv')
    train_filenames = pd.read_csv('/atlas/u/chenlin/wikipedia_images/Uganda_filtered_category_coordinate.csv')
    val_filenames = train_filenames
    labels_df = pd.read_csv('category_bounding_box.csv')
    arr = np.arange(len(train_filenames))
    np.random.shuffle(arr)

    # Prepare the training dataset
    file_names, file_labels = prepare_dataset.dataset_iterator(args.train_dir, train_filenames, arr, labels_df, similarity_df, 5000)
    ...
    y_train = tf.placeholder(tf.int32, [None, len(labels_df)])
    ...
    np.random.shuffle(arr)
    file_names, file_labels = prepare_dataset.dataset_iterator(args.test_dir, val_filenames, arr, labels_df, similarity_df, 50)
    ...
    with tf.variable_scope("FirstStageFeatureExtractor") as scope:
        ...
        out_1_max = tf.nn.max_pool(out_1, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME', name='max_pool')
    with tf.variable_scope("ResNetBlock1"):
        out_2 = net.resnet50_module(out_1_max, 3, 64, 64, 256, is_training, [1, 1, 1, 1])
    for index in range(20):
        tf.summary.image("ResNet_Block1_Feature_Maps", tf.expand_dims(out_2[:, :, :, index], -1),max_outputs=20)
    ...
    for index in range(20):
        tf.summary.image("ResNet_Block3_Feature_Maps", tf.expand_dims(out_4[:, :, :, index], -1), max_outputs=20)
    ...
    with tf.variable_scope("PredictionBlock"):
        ...
        y_pred = net._fcl(out_7, [1024, len(labels_df)], [len(labels_df)], 'fc_2', classification_layer=True)
    ...
    tf.summary.text('Labels', tf.as_string(y_train))
